...
{
	int queue_value = 1;//start from pos 1
...
{
	int queue_value = 1;//start from 1
	char *queue_value_tmp;
	char queue_name[300] = {0}; /* 队列名称的总长度，用户输入的队列长度少于256字节 */
...
/* 获取本次“入队列”操作的队列写入点 */
static int httpsqs_now_putpos(const char* httpsqs_input_name, bool priority)
{
	int maxqueue_num = 0;
	int queue_put_value = 0;
	int queue_get_value = 0;
	...
	int p=queue_put_value+1;
	if(p>maxqueue_num)
	{
		p=1;
	}
	if(p!=queue_get_value)//未满
	{
		if(priority)
		{
			pos=queue_get_value-1;	
			if(pos==0)
			{
				pos=maxqueue_num;
			}
		...
		else
		{
			pos=queue_put_value;
			queue_put_value++;
			if(queue_put_value>maxqueue_num)
			{
				queue_put_value=1;
			}
...
static int httpsqs_now_getpos(const char* httpsqs_input_name)
{
	...
	if(queue_get_value!=queue_put_value)
	{
		pos=queue_get_value;
		queue_get_value++;
		if(queue_get_value>maxqueue_num)
		{
			queue_get_value=1;
		}
		char queue_name[300] = {0}; /* 队列名称的总长度，用户输入的队列长度少于256字节 */
...
void httpsqs_handler(struct evhttp_request *req, void *arg)
{
		...
		const char *httpsqs_input_priority = evhttp_find_header (&httpsqs_http_query, "priority"); /* 队列总长度 字符型 */
		int httpsqs_input_pos = 0;
		int httpsqs_input_num = 0;

		bool pri=false;
		if((httpsqs_input_priority!=NULL) && (strcmp(httpsqs_input_priority,"true")==0))
		{
			pri=true;
		}
		...
		else  
		{
			...
			if (httpsqs_input_name != NULL && httpsqs_input_opt != NULL && strlen(httpsqs_input_name) <= 256) {
				...
				if (strcmp(httpsqs_input_opt, "put") == 0) {
					...
					if(httpsqs_input_postbuffer!=NULL)
					{
						int queue_put_value = httpsqs_now_putpos((char *)httpsqs_input_name, pri);
					...
					evbuffer_add_printf(buf, "%s", res);
					
				}
				/* 出队列 */
				else if (strcmp(httpsqs_input_opt, "get") == 0) {
					int queue_get_value = httpsqs_now_getpos((char *)httpsqs_input_name);
